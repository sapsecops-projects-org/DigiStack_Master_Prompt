...
> **Clarification for this project's scale (added 2026-07-19 cross-file audit):** "Side-by-Side" and "Parallel Environment" (v47) are realized here as **mixed-version cluster membership on the existing VM inventory** — during the rolling window, some cluster members run the old WAS version and some run the new, within the same cluster and the same VMs already listed in doc 01 — not as a second, wholly separate set of hardware standing up a duplicate environment. This is a deliberate scope decision consistent with this project's lab scale, not an oversight; a real enterprise migration at larger scale might well provision genuinely separate hardware for the "new" side, but no version in this Part introduces a new VM for that purpose (see each version's own "VM Setup Note" below), and none should be assumed to exist that isn't in doc 01.
...
### Topics Covered
- Blue-Green Migration, Canary Migration, Rolling Upgrade, Parallel Environment (old + new run simultaneously during transition — see v44's clarification above: realized as mixed-version cluster membership on the existing VM inventory, not separate new hardware)
...
- Added a clarification note to Version 44's "Migration Strategy Options" section, resolving the ambiguity between "Side-by-Side"/"Parallel Environment" language and the "no new VM introduced" VM Setup Notes throughout this Part — both now explicitly state this is realized as mixed-version cluster membership on the existing VM inventory, not separate new hardware.
- Added a short cross-reference to that same clarification in Version 47's Topics Covered list.
